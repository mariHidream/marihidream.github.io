...
import { Check, ChevronDown, ChevronUp, Languages, Loader2, Star } from "lucide-react";
...
export default function ReviewCard ({ review } : ReviewCardProps) {
    ...
    const [isExpanded, setIsExpanded] = useState(false); // 텍스트 확장 상태 추가
    ...
    // 현재 보여줄 텍스트 결정 
    const displayText = showTranslated && translatedText ? translatedText : review.content;
    
    // 텍스트 길이가 150자를 넘어가면 장문으로 간주하고 더보기 버튼 표출
        ...
        <div className="space-y-2">
                ...
                {isLongText && (
                    <button 
                        ...
                        className="text-teal-400 text-xs font-bold flex items-center gap-1 hover:text-teal-300 transition-colors"
                    >
