   ...
   const router = useRouter();
    ...
    return (
        <div className="relative w-full bg-slate-950 overflow-hidden theme-swiper-wrapper">
            ...
            >
                {movies.map((movie) => (
                    <SwiperSlide key={movie.id} className="relative w-full h-full flex items-center justify-center overflow-hidden">
                    
                    {/* 앰비언트 광원 레이어 (배경 블러 처리) */}
                    <div 
                        className="w-full absolute inset-0 bg-cover bg-no-repeat bg-top opacity-45 scale-110"
